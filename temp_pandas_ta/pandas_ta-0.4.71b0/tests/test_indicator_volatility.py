...
def test_bbands(df):
    ...
    assert isinstance(result, DataFrame)
    assert result.name == "BBANDS_5_2.0_2.0"

    try:
        expected = tal.BBANDS(df.close)
        expecteddf = DataFrame({
            "BBL_5_2.0_2.0": expected[2],
            "BBM_5_2.0_2.0": expected[1],
            "BBU_5_2.0_2.0": expected[0]
        })
    ...
    result = ta.bbands(df.close, lower_std=3, upper_std=5)
    assert isinstance(result, DataFrame)
    assert result.name == "BBANDS_5_3_5"
...
def test_hwc(df):
    ...
    result = ta.hwc(df.close, channels=False)
    assert isinstance(result, DataFrame)
    assert result.name == "HWC_1"
...
def test_ext_bbands(df):
    ...
    columns = [
        "BBL_5_2.0_2.0", "BBM_5_2.0_2.0",
        "BBU_5_2.0_2.0", "BBB_5_2.0_2.0", "BBP_5_2.0_2.0"
    ]
    assert list(df.columns[-5:]) == columns

    result = df.ta.bbands(lower_std=3, upper_std=5)
    assert isinstance(result, DataFrame)
    assert result.name == "BBANDS_5_3_5"
    columns = ["BBL_5_3_5", "BBM_5_3_5", "BBU_5_3_5", "BBB_5_3_5", "BBP_5_3_5"]
...
def test_ext_hwc(df):
    ...
    assert list(df.columns[-5:]) == [
        "HWM_1", "HWL_1", "HWU_1", "HWW_1", "HWPCT_1"
    ]
