...
def test_hma(df):
    ...
    result = ta.hma(df.close, mamode="ema")
    assert isinstance(result, Series)
    assert result.name == "HMAe_10"

    result = ta.hma(df.close, mamode="sma")
    assert isinstance(result, Series)
    assert result.name == "HMAs_10"
...
@mark.parametrize("kind,name", [
    ("dema", "DEMA_10"), ("ema", "EMA_10"), ("fwma", "FWMA_10"),
    ("hma", "HMA_10"), ("linreg", "LINREG_14"), ("midpoint", "MIDPOINT_2"),
    ("pwma", "PWMA_10"), ("rma", "RMA_10"), ("sinwma", "SINWMA_14"),
    ("sma", "SMA_10"), ("ssf", "SSF_20"), ("swma", "SWMA_10"),
    ("t3", "T3_10_0.7"), ("tema", "TEMA_10"), ("trima", "TRIMA_10"),
    ("vidya", "VIDYA_14"), ("wma", "WMA_10")
])
def test_ma(df, kind, name):
# def test_ma(df):
    result = ta.ma()
    assert isinstance(result, list)
    assert len(result) > 0

    result = ta.ma(kind, df.close)
    assert isinstance(result, Series)
...
def test_vidya(df):
    result = ta.vidya(df.close, talib=False)
    assert isinstance(result, Series)
    assert result.name == "VIDYA_14"

    try:
        expected = ta.vidya(df.close, talib=True)
        assert isinstance(expected, Series)
        assert expected.name == "VIDYA_14"
        pdt.assert_series_equal(result, expected, check_names=False)
    except AssertionError:
        try:
            corr = ta.utils.df_error_analysis(result, expected)
...
def test_ext_alligator(df):
    ...
    assert list(df.columns[-3:]) == [
        "AGj_13_8_5", "AGt_13_8_5", "AGl_13_8_5"
    ]
