...
def test_qqe(df):
    result = ta.qqe(df.close, asmode=True)
    assert isinstance(result, DataFrame)
...
def test_squeeze_pro(df):
    ...
    result = ta.squeeze_pro(df.high, df.low, df.close, 20, 2, 20, 1, 2, 3)
    assert isinstance(result, DataFrame)
    assert result.name == "SQZPRO_20_2_20_3.0_2.0_1.0"

    result = ta.squeeze_pro(df.high, df.low, df.close, 20, 2, 20, 1, 2, 3, tr=False)
    assert isinstance(result, DataFrame)
...
def test_ext_dm(df):
    ...
    assert list(df.columns[-2:]) == ["DMP_14", "DMN_14"]
