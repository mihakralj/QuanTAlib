<<<<<<< Updated upstream:QuantLib/Series/SMA_Series.cs
﻿using System;
...
public class SMA_Series : TSeries {
  ...
  public new void Add((System.DateTime t, double v)data, bool update = false) {
    ...
    for (int i = 0; i < this._buffer.Count; i++) {
      ...
      _mse_item +=
          (this._buffer[i] - _sma_item) * (this._buffer[i] - _sma_item);
      _mape_item +=
          (this._buffer[i] != 0)
              ? Math.Abs((this._buffer[i] - _sma_item) / this._buffer[i])
              : double.PositiveInfinity;
    }
    var _stddev_item = (this._buffer.Count > 1)
                           ? Math.Sqrt(_mse_item / (this._buffer.Count - 1))
                           : 0;
    ...
    (System.DateTime t, double v) result =
        (data.t,
         (this.Count < this._p - 1 && this._NaN) ? double.NaN : _sma_item);
    (System.DateTime t, double v) stddev =
        (data.t,
         (this.Count < this._p - 1 && this._NaN) ? double.NaN : _stddev_item);
    (System.DateTime t, double v) mad =
        (data.t,
         (this.Count < this._p - 1 && this._NaN) ? double.NaN : _mad_item);
    (System.DateTime t, double v) mse =
        (data.t,
         (this.Count < this._p - 1 && this._NaN) ? double.NaN : _mse_item);
    (System.DateTime t, double v) mape =
        (data.t,
...
>>>>>>> Stashed changes:Algos/SMA_Series.cs
