<<<<<<< Updated upstream:QuantLib/Series/JMA_Series.cs
﻿using System;
namespace QuantLib;
...
deduction and reverse analysis of JMA behavior. It is close, but not exact JMA -
the tests against JMA.CSV fail with small deviation. :-(
**/
public class JMA_Series : TSeries {
  ...
  private double o_prev_ma1, o_prev_det0, o_prev_det1, o_prev_jma, o_bsmax,
      o_bsmin;

  private readonly double pr, pow1, len2, beta, rvolty;
  private readonly int _l;

  public JMA_Series(TSeries source, int period, double phase = 0.0,
                    bool useNaN = false) {
    this._p = period;
    this._NaN = useNaN;
    this._source = source;
  ...
  public new void Add((System.DateTime t, double v)data, bool update = false) {
    if (this.Count == 0) {
      this.prev_ma1 = this.prev_jma = data.v;
    ...
    for (int i = 0; i <= Math.Min(9, this._source.Count - 1); i++) {
      var _item = this._source[this._source.Count - 1 - i].v;
    ...
    double ma1 = data.v * (1 - alpha) + this.prev_ma1 * alpha;
    this.prev_ma1 = ma1;

    // 2nd stage - one more preliminary smoothing by Kalman filter
    double det0 = (data.v - ma1) * (1 - this.beta) + this.prev_det0 * this.beta;
    this.prev_det0 = det0;
...
public class JMA_Series : TSeries
{
    ...
    public JMA_Series(TSeries source, int period, double phase = 0.0, bool useNaN = false)
    {
        ...
        double _pp = this._p * 0.5;
        this.pr = (phase < -100) ? 0.5 : (phase > 100) ? 2.5 : (phase * 0.01) + 1.5;
        double len1 = Math.Max((Math.Log(Math.Sqrt(0.5 * (_pp - 1))) / Math.Log(2.0)) + 2.0, 0);
        this.pow1 = Math.Max(len1 - 2, 0.5);
        this.rvolty = Math.Exp((1 / this.pow1) * Math.Log(len1));
        this.len2 = Math.Sqrt(0.5 * (_pp - 1)) * len1;
        this.beta = 0.45 * (_pp - 1) / (0.45 * (_pp - 1) + 2);
    ...
    public new void Add((System.DateTime t, double v) data, bool update = false)
    {
        ...
        double pow2 = Math.Exp(this.pow1 * Math.Log(dvolty));
        double kv = Math.Exp(Math.Sqrt(pow2) * Math.Log(this.len2 / (this.len2 + 1)));
        ...
        // 3rd stage - final smoothing by Jurik adaptive filter
        double det1 = ((ma2 - this.prev_jma) * (1 - alpha) * (1 - alpha)) + (this.prev_det1 * alpha * alpha);
        this.prev_det1 = det1;
        var jma = this.prev_jma + det1;
        this.prev_jma = jma;
...
>>>>>>> Stashed changes:Algos/JMA_Series.cs
