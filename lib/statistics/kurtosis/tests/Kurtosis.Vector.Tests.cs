...
public sealed class KurtosisVectorTests
{
    private static readonly int[] Periods = [4, 5, 8, 16];

    private readonly ITestOutputHelper _output;

    public KurtosisVectorTests(ITestOutputHelper output)
    {
        _output = output;
    }
    ...
    public void VectorCore_MatchesScalarCore_AllFinite()
    {
        ...
        {
            Verify(BuildInput(1024, seed: 1001, withNonFinite: false), period, isPopulation: false);
            Verify(BuildInput(1024, seed: 1001, withNonFinite: false), period, isPopulation: true);
        }
    }
    ...
    public void VectorCore_MatchesScalarCore_WithNonFinite()
    {
        foreach (int period in Periods)
        {
            Verify(BuildInput(1024, seed: 1002, withNonFinite: true), period, isPopulation: false);
            Verify(BuildInput(1024, seed: 1002, withNonFinite: true), period, isPopulation: true);
        }
    }
    ...
    public void VectorCore_MatchesScalarCore_EdgeValues()
    {
        ...
        foreach (double[] source in cases)
        {
            foreach (int period in Periods)
            {
                Verify(source, period, isPopulation: false);
                Verify(source, period, isPopulation: true);
            }
        }
    }
    ...
    public void VectorCore_MatchesScalarCore_ShortArrays()
    {
        ...
        for (int n = 0; n <= limit; n++)
        {
            foreach (int period in Periods)
            {
                Verify(BuildInput(n, seed: 2000 + n, withNonFinite: false), period, isPopulation: false);
                Verify(BuildInput(n, seed: 3000 + n, withNonFinite: true), period, isPopulation: true);
            }
        }
    }

    private void Verify(double[] source, int period, bool isPopulation)
    {
        double[] scalar = new double[source.Length];
        double[] vector = new double[source.Length];
        Kurtosis.CalculateScalarCore(source, scalar, period, isPopulation);
        Kurtosis.CalculateVectorCore(source, vector, period, isPopulation);
        double maxDiff = Compare(scalar, vector);
        _output.WriteLine($"n={source.Length} period={period} pop={isPopulation} maxAbsDiff={maxDiff:G17}");
    }
