...
public class BetadistTests
{
    ...
    {
        var ex = Assert.Throws<ArgumentException>(() => new Betadist(period: 0));
    ...
    {
        var ex = Assert.Throws<ArgumentException>(() => new Betadist(period: -1));
        Assert.Equal("period", ex.ParamName);
    }
    ...
    {
        var ex = Assert.Throws<ArgumentException>(() => new Betadist(alpha: 0.0));
    ...
    {
        var ex = Assert.Throws<ArgumentException>(() => new Betadist(alpha: -1.0));
        Assert.Equal("alpha", ex.ParamName);
    }
    ...
    {
        var ex = Assert.Throws<ArgumentException>(() => new Betadist(beta: 0.0));
    ...
    {
        var ex = Assert.Throws<ArgumentException>(() => new Betadist(beta: -0.5));
        Assert.Equal("beta", ex.ParamName);
    }
    ...
    public void Batch_Span_EmptySource_ThrowsArgumentException()
    {
        var ex = Assert.Throws<ArgumentException>(() =>
            Betadist.Batch([], Array.Empty<double>()));
