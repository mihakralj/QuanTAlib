...
public class LogTests
{
    ...
    public void Constructor_SetsProperties()
    {
        ...
        Assert.Equal("Log", indicator.Name);
    ...
    public void Calculate_TSeries_MatchesStreaming()
    {
        ...
        // Batch
        var batch = Log.Calculate(source);
    ...
    public void Calculate_Span_MatchesTSeries()
    {
        ...
        // TSeries batch
        var batchResult = Log.Calculate(source);
    ...
    public void Calculate_Span_ValidatesArguments()
    {
        Assert.Throws<ArgumentException>(() =>
        {
            ...
            Log.Calculate(ReadOnlySpan<double>.Empty, output);
        });
        ...
        Assert.Throws<ArgumentException>(() =>
        {
            ...
            Log.Calculate(source, output);
        });
    }

    [Fact]
    public void LogExpInverse_ReturnsOriginal()
    {
        var log = new Log();
        var time = DateTime.UtcNow;
        double original = 42.0;

        log.Update(new TValue(time, original));
        double logResult = log.Last.Value;

        // exp(log(x)) should equal x
        Assert.Equal(original, Math.Exp(logResult), Tolerance);
    }
}
