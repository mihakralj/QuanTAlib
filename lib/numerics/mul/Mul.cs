...
// Combines two live streams into their product: result = a * b.
...
/// </remarks>
/// <seealso href="Mul.md">Detailed documentation</seealso>
[SkipLocalsInit]
public sealed class Mul : BiInputIndicatorBase
{
    /// <summary>
    /// Creates a Mul node for direct pull-style updates.
    /// </summary>
    public Mul() : base(1, "Mul")
    {
    }

    /// <summary>
    /// Creates a Mul node chained to two upstream publishers using the time-join rule.
    /// </summary>
    /// <param name="a">First factor publisher</param>
    /// <param name="b">Second factor publisher</param>
    public Mul(ITValuePublisher a, ITValuePublisher b) : this()
    {
        Subscribe(a, b);
    }
    ...
    public static void Batch(ReadOnlySpan<double> a, ReadOnlySpan<double> b, Span<double> output)
    {
        ...
        SimdExtensions.Multiply(a, b, output);
    }

    private static void BatchDelegate(ReadOnlySpan<double> a, ReadOnlySpan<double> b, Span<double> output, int period) =>
        Batch(a, b, output);

    public static (TSeries Results, Mul Indicator) Calculate(TSeries a, TSeries b)
    {
        var indicator = new Mul();
