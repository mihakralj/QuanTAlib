...
public sealed class Notch : AbstractBase
{
    ...
    public Notch(int period, double q = 1.0)
    {
        ...
        if (q <= 0)
        {
            throw new System.ArgumentOutOfRangeException(nameof(q), "Q factor must be positive.");
        }
        ...
        double omega = System.Math.PI * 2.0 / period;
        double sn = System.Math.Sin(omega);
        double cs = System.Math.Cos(omega);
    ...
    public override void Prime(ReadOnlySpan<double> source, TimeSpan? step = null)
    {
        ...
        {
            Update(new TValue(System.DateTime.MinValue, value));
        }
    }
    ...
    public override TValue Update(TValue input, bool isNew = true)
    {
        ...
        double y = System.Math.FusedMultiplyAdd(_b0, val, 0);
        y = System.Math.FusedMultiplyAdd(_b1, _state.X1, y);
        y = System.Math.FusedMultiplyAdd(_b2, _state.X2, y);
        y = System.Math.FusedMultiplyAdd(-_a1, _state.Y1, y);
        y = System.Math.FusedMultiplyAdd(-_a2, _state.Y2, y);
    ...
    public static void Calculate(ReadOnlySpan<double> source, Span<double> output, int period, double q = 1.0)
    {
        ...
        {
            throw new System.ArgumentException("Source and output lengths must match.", nameof(output));
        }
        ...
        for (int i = 0; i < source.Length; i++)
        {
            ...
            double y = System.Math.FusedMultiplyAdd(b0, val, 0);
            y = System.Math.FusedMultiplyAdd(b1, x1, y);
            y = System.Math.FusedMultiplyAdd(b2, x2, y);
            y = System.Math.FusedMultiplyAdd(-a1, y1, y);
            y = System.Math.FusedMultiplyAdd(-a2, y2, y);
