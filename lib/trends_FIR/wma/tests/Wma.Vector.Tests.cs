...
public sealed class WmaVectorTests
{
    private static readonly int[] Periods = [1, 2, 3, 4, 5, 8, 16];

    private readonly ITestOutputHelper _output;

    public WmaVectorTests(ITestOutputHelper output)
    {
        _output = output;
    }
    ...
    private void Verify(double[] source, int period)
    {
        ...
        Wma.CalculateScalarCore(source, scalar, period);
        Wma.CalculateVectorCore(source, vector, period);
