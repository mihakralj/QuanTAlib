...
public sealed class Hanma : AbstractBase
{
    ...
    /// <summary>
    /// Creates HANMA with specified parameters.
    /// </summary>
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Hanma(ITValuePublisher source, int period = 10) : this(period)
    {
        _source = source;
        _pubHandler = Handle;
    ...
    /// <summary>
    /// Computes Hanning window weights.
    /// w[i] = 0.5 * (1 - cos(2πi/(period-1)))
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    private double CalculateWeightedSum()
    {
        ...
        if (count < _period)
        {
            ...
            // Hanning edge case: when weight sum is 0 (e.g., only edge values in buffer),
            // return simple average of buffered values
            if (wSum <= 0)
            {
                double avg = 0;
                for (int i = 0; i < count; i++)
    ...
    /// <summary>
    /// Calculates HANMA from a TSeries using streaming updates.
    /// </summary>
    public static TSeries Batch(TSeries source, int period = 10)
    {
        var hanma = new Hanma(period);
        return hanma.Update(source);
    }

    /// <summary>
    /// Calculates HANMA over a span of values (SIMD-optimized for batch processing).
    /// </summary>
