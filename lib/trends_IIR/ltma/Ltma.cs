...
/// </summary>
/// <remarks>
/// Dual cascaded EMAs with linear trend extrapolation. Estimates the per-bar
/// slope from the spread between two cascaded EMAs and projects forward by
/// one lag interval, producing <c>LTMA = 2·EMA1 − EMA2</c>.
///
/// Differs from DEMA only in warmup strategy (first-bar seeding in Pine;
/// compensator in C# for library consistency).
/// </remarks>
/// <seealso href="Ltma.md">Detailed documentation</seealso>
/// <seealso href="ltma.pine">Reference Pine Script implementation</seealso>
[SkipLocalsInit]
public sealed class Ltma : AbstractBase
{
    ...
    public Ltma(ITValuePublisher source, int period) : this(period)
    {
        _publisher = source;
        _listener = Handle;
    ...
    public override TValue Update(TValue input, bool isNew = true)
    {
        ...
        // Sanitize input — last-valid-value substitution
        ...
        // EMA2 (cascaded off EMA1)
        double e2 = Compute(e1, _alpha, _decay, ref _state2);

        // LTMA = 2·EMA1 − EMA2 (one-lag-interval extrapolation)
    ...
    public static (TSeries Results, Ltma Indicator) Calculate(TSeries source, int period)
    {
        var indicator = new Ltma(period);
