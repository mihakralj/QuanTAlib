...
public sealed class UoValidationTests : IDisposable
{
    private readonly ITestOutputHelper _output;
    private readonly ValidationTestData _testData;
    private const int DefaultBandEdge = 20;

    public UoValidationTests(ITestOutputHelper output)
    {
        _output = output;
    ...
    [Fact]
    public void Uo_BatchStreaming_Match()
    {
        // Streaming
        var streaming = new Uo(DefaultBandEdge);
        ...
        // Batch
        TSeries batchResults = Uo.Batch(_testData.Data, DefaultBandEdge);
        ...
        _output.WriteLine($"Uo({DefaultBandEdge}) Batch vs Streaming: {mismatchCount} mismatches, max diff = {maxDiff:E3}");
        Assert.Equal(0, mismatchCount);
    }

    [Fact]
    public void Uo_SpanBatch_MatchesStreaming()
    {
        // Streaming
        var streaming = new Uo(DefaultBandEdge);
        var streamResults = new List<double>(_testData.Data.Count);
        ...
        Uo.Batch(_testData.Data.Values, output, DefaultBandEdge);
        ...
        _output.WriteLine($"Uo({DefaultBandEdge}) Span vs Streaming: {mismatchCount} mismatches, max diff = {maxDiff:E3}");
        Assert.Equal(0, mismatchCount);
    }

    [Fact]
    public void Uo_DifferentBandEdges_ProduceDifferentResults()
    {
        TSeries result10 = Uo.Batch(_testData.Data, 10);
        TSeries result40 = Uo.Batch(_testData.Data, 40);

        int lastIdx = _testData.Data.Count - 1;
        _output.WriteLine($"Uo(10) last = {result10[lastIdx].Value:F6}");
        _output.WriteLine($"Uo(40) last = {result40[lastIdx].Value:F6}");

        Assert.NotEqual(result10[lastIdx].Value, result40[lastIdx].Value);
    }

    [Fact]
    public void Uo_ConstantInput_ConvergesToZero()
    {
        ...
        _output.WriteLine($"Uo(10) constant input result after 1000 bars: {lastResult:E6}");
        Assert.True(Math.Abs(lastResult) < 1e-6, $"Expected near-zero for constant input, got {lastResult}");
    }

    [Fact]
    public void Uo_Calculate_ReturnsHotIndicator()
    {
        (TSeries results, Uo indicator) = Uo.Calculate(_testData.Data, DefaultBandEdge);
        ...
        _output.WriteLine($"Uo({DefaultBandEdge}) Calculate: {results.Count} bars, last = {results[results.Count - 1].Value:F6}");
    }

    [Fact]
    public void Uo_BarCorrection_ProducesConsistentResults()
    {
        // Build reference: 100 bars then bar 101
        var reference = new Uo(DefaultBandEdge);
        for (int i = 0; i < 100; i++)
        {
        ...
        var test = new Uo(DefaultBandEdge);
        for (int i = 0; i < 100; i++)
        {
    ...
    [Fact]
    public void Uo_SubsetValidation_StableBehavior()
    {
        using var subset = _testData.CreateSubset(200);

        TSeries results = Uo.Batch(subset.Data, DefaultBandEdge);
        ...
        _output.WriteLine($"Uo({DefaultBandEdge}) on 200-bar subset: {nanCount} non-finite values");
