// PIVOTWOOD Validation Tests - Woodie's Pivot Points
...
// No external library implements Woodie's Pivot Points with bar-to-bar granularity.
...
public sealed class PivotwoodValidationTests
{
    ...
    // ── Math correctness ────────────────────────────────────────────
    [Fact]
    public void MathCorrectness_WoodieFormula()
    {
    ...
    {
        var bars = CreateGbmBars();
        var (results, indicator) = Pivotwood.Calculate(bars);
    ...
    public void LevelOrdering_S3_LessThan_S2_LessThan_S1_LessThan_PP_LessThan_R1_LessThan_R2_LessThan_R3()
    {
        ...
        for (int i = 0; i < bars.Count; i++)
        {
            ...
            // For Woodie's pivots with positive range, strict ordering holds
    ...
    // ── Close-weight bias ───────────────────────────────────────────
    [Fact]
    public void CloseWeightBias_PP_DiffersFromClassicPivot()
    {
        var bars = CreateGbmBars();
        var woodie = new Pivotwood();
        var classic = new Pivot();

        for (int i = 0; i < bars.Count; i++)
        {
            woodie.Update(bars[i], isNew: true);
            classic.Update(bars[i], isNew: true);

            if (!woodie.IsHot) { continue; }

            double pH = bars[i - 1].High;
            double pL = bars[i - 1].Low;
            double pC = bars[i - 1].Close;

            // When close != (H+L)/2, Woodie and Classic PP differ
            double midpoint = (pH + pL) * 0.5;
            if (Math.Abs(pC - midpoint) > 1e-10)
            {
                Assert.NotEqual(woodie.PP, classic.PP);
            }
        }
    }
}
