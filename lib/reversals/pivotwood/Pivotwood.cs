...
// Calculates 7 support/resistance levels from previous bar's HLC with close weighted 2×.
// Woodie's formula weights the closing price twice, biasing the pivot toward recent action.
...
/// high, low, and close. Unlike classic pivots where PP = (H+L+C)/3,
/// Woodie's formula weights the close price twice: PP = (H+L+2C)/4.
/// This biases the pivot toward the most recent closing price, giving
/// more weight to where the market settled.
///
/// Calculation (using previous bar's H, L, C):
/// <code>
/// PP = (H + L + 2*C) / 4          (weighted close)
/// R1 = 2*PP - L                    S1 = 2*PP - H
/// R2 = PP + (H - L)                S2 = PP - (H - L)
...
/// - Close-weighted PP biases toward recent price action
/// </remarks>
/// <seealso href="Pivotwood.md">Detailed documentation</seealso>
[SkipLocalsInit]
public sealed class Pivotwood : ITValuePublisher
{
    ...
    /// <summary>Support 2: PP - range</summary>
    ...
    /// <summary>
    /// Creates a Woodie's Pivot Points indicator.
    /// </summary>
    public Pivotwood()
    {
        _count = 0;
    ...
    /// <summary>
    /// Creates a Woodie's Pivot Points indicator chained to a TBarSeries source.
    /// </summary>
    public Pivotwood(TBarSeries source)
        : this()
    {
        Prime(source);
    ...
    public TValue Update(TBar input, bool isNew = true)
    {
        ...
        // Woodie's PP: weight close 2× → (H + L + 2C) / 4
        double pp = (pH + pL + Math.FusedMultiplyAdd(2.0, pC, 0.0)) * 0.25;
        double range = pH - pL;

        PP = pp;
        R1 = Math.FusedMultiplyAdd(2.0, pp, -pL);    // 2*PP - L
        S1 = Math.FusedMultiplyAdd(2.0, pp, -pH);    // 2*PP - H
        R2 = pp + range;                               // PP + range
        S2 = pp - range;                               // PP - range
        R3 = Math.FusedMultiplyAdd(2.0, pp - pL, pH); // H + 2*(PP - L)
        S3 = Math.FusedMultiplyAdd(-2.0, pH - pp, pL); // L - 2*(H - PP)
    ...
    /// <summary>
    /// Batch computation of Woodie's Pivot Points over span data.
        ...
        Span<double> ppOutput)
    {
        ...
        // Remaining bars: compute Woodie's PP from previous bar's HLC
        for (int i = 1; i < len; i++)
        {
            double pH = high[i - 1];
            double pL = low[i - 1];
            double pC = close[i - 1];
            ppOutput[i] = (pH + pL + Math.FusedMultiplyAdd(2.0, pC, 0.0)) * 0.25;
        }
    }
    ...
    /// <summary>
    /// Batch computation of all 7 Woodie's Pivot Point levels over span data.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    public static (TSeries Results, Pivotwood Indicator) Calculate(TBarSeries source)
    {
        var indicator = new Pivotwood();
