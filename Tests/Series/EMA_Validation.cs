...
public class EMA_Validation
{
    ...
    public void Skender_Stock()
    {
        ...
        EMA_Series QL = new(bars.Close, period, false);
        var SK = quotes.GetEma(period, CandlePart.Close);
        double expected = Math.Round((double)SK.Last().Ema!, 8);
    ...
    public void TA_LIB()
    {
        ...
        EMA_Series QL = new(bars.Close, period, false);
        Core.Ema(input, 0, bars.Count - 1, TALIB, out int outBegIdx, out int outNbElement, period);
    ...
    public void Pandas_TA()
    {
    	...
    	EMA_Series QL = new(bars.Close, period, false);
    	var pta = ta.ema(close: df[0], length: period);
